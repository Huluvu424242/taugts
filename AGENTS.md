...
- Der Über-Dialog zeigt das App-Logo unmittelbar links neben der `Schließen`-Aktion. Das Logo wird so skaliert und ohne zusätzlichen Layout-Footprint eingebunden, dass der bestehende Abstand zwischen `Schließen` und der benachbarten Aktion für die Barrierefreiheitserklärung unverändert bleibt.
