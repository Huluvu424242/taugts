...
void main() {
  ...
  test('zählt neue, unveränderte und geänderte Datensätze nach UUID', () {
    ...
    final analyse =
        service.analysiere(importDokument: import, lokalesDokument: lokal);
  ...
  test('zusätzliche Bewertung mit anderer UUID bleibt neuer Historieneintrag',
    ...
    final lokal = dokument(bewertungen: [
      {
        'id': 'bew-1',
        'objektId': 'obj-1',
        'erstelltAm': '2026-01-01T10:00:00Z'
      },
    ]);
    ...
    final analyse =
        service.analysiere(importDokument: import, lokalesDokument: lokal);
    final bewertungen =
        analyse.sammlungen.singleWhere((e) => e.name == 'Bewertungen');
  ...
  test('erkennt fachliche Produkt- und Ortsdubletten nachvollziehbar', () {
    ...
    expect(analyse.fachlicheDubletten.every((e) => e.begruendung.isNotEmpty),
        isTrue);
  });

  test('macht eigene und fremde Herkunft sichtbar', () {
    final lokal = dokument(profile: [
      {'id': 'eigen'}
    ]);
