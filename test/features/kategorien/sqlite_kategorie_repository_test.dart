...
void main() {
  ...
  test('liefert die reproduzierbare Standardhierarchie', () {
    expect(
      repository.finde(StandardKategorien.bier.id)?.elternId,
      StandardKategorien.getraenk.id,
    );
    expect(
      repository.finde(StandardKategorien.speise.id)?.elternId,
      StandardKategorien.produkt.id,
    );
    expect(
      repository.finde(StandardKategorien.gastronomie.id)?.elternId,
      StandardKategorien.ort.id,
    );
    expect(
      repository.finde(StandardKategorien.geschaeft.id)?.elternId,
      StandardKategorien.ort.id,
    );
  });
  ...
  test('verhindert stilles Löschen verwendeter Kategorien', () {
    ...
    expect(
      ...
      contains(StandardKategorien.bier.id),
    );
    expect(
