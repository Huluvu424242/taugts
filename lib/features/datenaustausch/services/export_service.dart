...
class ExportService {
  ...
  String erzeugeJson() {
    ...
    return const JsonEncoder.withIndent('  ').convert(dokument);
  }
  ...
  List<Map<String, Object?>> _objekte() {
    final zeilen = datenbank.verbindung.select('''
      SELECT o.*, p.marke, p.produktart, p.brauerei, p.sorte,
        ...
        p.barcode, p.notiz
      FROM objekte o JOIN produkte p ON p.objekt_id = o.id
      ORDER BY o.id
    ''');
    return zeilen.map((z) => {
          'id': z['id'],
  ...
  List<Map<String, Object?>> _erlebnisPositionen() =>
      _zeilen('erlebnispositionen').map((z) => {
            'id': z['id'],
  ...
  List<Map<String, Object?>> _preisbeobachtungen() =>
      _zeilen('preisbeobachtungen').map((z) => {
            'id': z['id'],
            'erlebnisId': z['erlebnis_id'],
  ...
  List<Map<String, Object?>> _bewertungen() => _zeilen('bewertungen').map((z) => {
        'id': z['id'],
        'zielart': z['ortsbewertung_id'] == null ? 'produkt' : 'ort',
        'objektId': z['ortsbewertung_id'] == null ? _produktId(z['erlebnis_position_id']) : null,
        'erlebnisId': z['erlebnis_id'],
        'erlebnisPositionId': z['erlebnis_position_id'],
  ...
  List<String> _auswahlwerte(Object? wert) {
    ...
    return text.split('|').where((element) => element.isNotEmpty).toList();
  }
}
