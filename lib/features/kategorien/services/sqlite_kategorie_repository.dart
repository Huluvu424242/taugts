...
class SqliteKategorieRepository implements KategorieRepository {
  ...
  Kategorie? finde(String id) {
    final result = _db.verbindung.select(
  ...
  Kategorie _ausZeile(Map<String, Object?> zeile) => Kategorie(
  ...
  void speichern(Kategorie kategorie) {
    ...
    _db.verbindung.execute(
      ...
      '(id, name, bereich, eltern_id, ist_standard) VALUES (?, ?, ?, ?, ?)',
      [
        kategorie.id,
        kategorie.name.trim(),
        kategorie.bereich.name,
        kategorie.elternId,
        kategorie.istStandard ? 1 : 0,
      ],
    );
  }
  ...
  void _pruefeEltern(
    String id,
    KategorieBereich bereich,
    String? elternId,
  ) {
    if (elternId == null) return;
    if (elternId == id) {
      throw const UngueltigeKategorieHierarchieException(
        'Eine Kategorie kann nicht ihr eigener Elternknoten sein.',
      );
    }
    final eltern = finde(elternId);
    if (eltern == null || eltern.bereich != bereich) {
      throw const UngueltigeKategorieHierarchieException(
        'Elternkategorie fehlt oder gehört zu einem anderen Bereich.',
      );
    }
    ...
    while (aktuell != null) {
      if (aktuell == id) {
        throw const UngueltigeKategorieHierarchieException(
          'Die Zuordnung würde einen Zyklus erzeugen.',
        );
      }
  ...
  void _ordneZu(
    ...
    _db.verbindung.execute(
      'INSERT OR IGNORE INTO $tabelle '
      '($objektSpalte, kategorie_id) VALUES (?, ?)',
      [objektId, kategorieId],
    );
  }

  @override
  void ordneProduktZu(String produktId, String kategorieId) => _ordneZu(
        'produkt_kategorien',
        'produkt_id',
        produktId,
        kategorieId,
        KategorieBereich.produkt,
      );

  @override
  void ordneOrtZu(String ortId, String kategorieId) => _ordneZu(
        'ort_kategorien',
        'ort_id',
        ortId,
        kategorieId,
        KategorieBereich.ort,
      );

  Set<String> _zuordnungen(String tabelle, String spalte, String id) =>
      _db.verbindung
  ...
  Set<String> kategorienFuerProdukt(String produktId) =>
      _zuordnungen('produkt_kategorien', 'produkt_id', produktId);

  @override
  Set<String> kategorienFuerOrt(String ortId) =>
      _zuordnungen('ort_kategorien', 'ort_id', ortId);
  ...
  void entfernen(String id) {
    ...
    if (benutzt) {
      throw KategorieInBenutzungException(id);
    }
